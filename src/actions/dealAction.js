...
export const createDeal = (payload, token, history) => async (dispatch) => {
    try {
        ...
        const response = await axios.post("https://quickhouse-436caeb406a0.herokuapp.com/api/order",
            payload,
            {
                headers: {
                    token: token,
...
export const getDeals = () => async (dispatch) => {
    try {
        ...
        const response = await axios.get(`https://quickhouse-436caeb406a0.herokuapp.com/api/deals?page=1&limit=25`)
        dispatch({
            type: DEAL_SUCCESS,
...
export const getSingleDeal = (id) => async (dispatch) => {
    try {
        ...
        const response = await axios.get(`https://quickhouse-436caeb406a0.herokuapp.com/api/deal/${id}`,)
        dispatch({
...
export const personDeal = (requesterId, token) => async (dispatch) => {
    try {
        ...
        const response = await axios.get(`https://quickhouse-436caeb406a0.herokuapp.com/api/myDeals?page=1&limit=10&requesterId=${requesterId}`, {
            headers: {
                token: token
            }
        })
